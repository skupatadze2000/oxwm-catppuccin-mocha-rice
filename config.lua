...
local bar_font = "monospace:style=Bold:size=15"
...
local blocks = {
    ...
    format = "Vol: {}%",
    command = "amixer sget Master | awk -F'[][]' '/%/ {print $2}' | tr -d '%'",
    interval = 5,
    color = colors.green,
    -- Add this to your battery block logic if you want to click to toggle mute
    -- click = "amixer set Master toggle",
    }),
    oxwm.bar.block.ram({
        format = "RAM: {used} G",
        interval = 5,
    ...
    oxwm.bar.block.shell({
    format = "Temp: {}°C",
    command = "cat /sys/class/thermal/thermal_zone0/temp | cut -c1-2",
    interval = 10,
    color = colors.cyan,
    }),
    oxwm.bar.block.shell({
...
-- Unoccupied tags
oxwm.bar.set_scheme_normal(colors.grey, colors.bg, colors.black)
-- Occupied tags
oxwm.bar.set_scheme_occupied(colors.white, colors.bg, colors.black)
...
oxwm.key.bind({ modkey }, "T", oxwm.spawn_terminal("st"))
-- Launch Dmenu
oxwm.key.bind({ modkey }, "D", oxwm.spawn({ "sh", "-c", "rofi  -show drun 10" }))
